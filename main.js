...
require([
	...
	$.ajax({
		...
		var models = _.pluck(resources.apis, 'path');
		var EndointArray = []
		...
		$.when.apply($, EndointArray).then(function () {
			console.log(arguments);
			_.each(arguments, function (argument) {
				argument.group = argument[0].apis[0].path.split('/')[1];
				argument[0].apis.forEach(function (endpoint) {
					if (endpoint.path.indexOf('{fk}') === -1) {
						endpoint.operations.forEach(function (operation) {
							var apiItem = {
								url: endpoint.path,
								title: operation.summary,
								type: operation.method.toLowerCase(),
